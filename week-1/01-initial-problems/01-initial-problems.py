...
def gas_stations(distance, km_per_tank, stations):
    index = 0
    nearest_stations = []
    while km_per_tank <= distance:
        while True:
            current_station_distance = stations[index]
            if current_station_distance >= km_per_tank:
                index -= 1
                nearest_stations.append(stations[index])
                break
            index += 1
